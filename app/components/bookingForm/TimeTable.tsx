...
import {addMinutes, format, fromUnixTime, isBefore, isAfter, parse} from 'date-fns'
...
export async function TimeTable({ selectedDate, userName, duration }: iAppProps) {
  ...
  const availableSlots = calculateAvailableTimeSlots(formattedDate, dbAvailability, nylasCalendarData, duration);
  ...
  return(
    <div className="ml-4">
      <p className="text-base font-semibold">
        ...
        <span className="text-sm text-muted-foreground">{format(selectedDate, "YYY")}</span>
      </p>

      <div className="mt-3 max-h-[350px] overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-track-white scrollbar-thumb-purple-500">
        {availableSlots.length > 0 ? (
          availableSlots.map((slot, index) => (
            <Link href={`?date=${format(selectedDate, "yyyy-MM-dd")}&time=${slot}`} key={index}>
              <Button variant="outline" className="w-full mb-2 mr-2">{slot}</Button>
            </Link>
          ))
        ) : (
